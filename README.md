...
`opti` (v0.3.3) scans a directory of H.264 / AVC videos, transcodes them to H.265 / HEVC with `ffmpeg`, and keeps track of progress so long-running batches can resume safely. It ships with a live terminal dashboard, parallel workers, and automatically replaces original files with transcoded versions.

## Features
- Automatic file discovery with `ffprobe`, limited to H.264 footage.
- Parallel `ffmpeg` transcodes (`-j` workers) with CPU (`libx265`), Intel Quick Sync (`-engine qsv`), or NVIDIA NVENC (`-engine nvenc`) when your ffmpeg build exposes the encoder.
...
- Automatically replaces original files with transcoded versions after successful encoding.
- Flat working directory structure - all transcoding happens in the root of the working directory for simplified temporary file management.
...
`opti` expects a **source** directory containing the original videos and a **work** directory where it can store temporary transcoding files and state. After successful transcoding, original files are automatically replaced with the HEVC versions.
...
| `-engine` | Transcode engine. Always accepts `cpu`; supports `qsv` for Intel Quick Sync and `nvenc` when `ffmpeg` exposes those encoders (see `-list-hw`). | `cpu` |
| `-ffmpeg` | Path to the `ffmpeg` binary. A sibling `ffprobe` will be used if found. | `ffmpeg` |
| `-ffprobe` | Explicit path to the `ffprobe` binary. Falls back to the `-ffmpeg` sibling or system `ffprobe` when not provided. | *(auto)* |
...
| `-debug` | Verbose logging for file discovery, ffprobe calls, and queue decisions. | `false` |
...
4. Finished encodes are written directly to the root of the work directory (no subdirectories), appending `.hevc.mkv` by default. Use `-output-mp4` to force `.hevc.mp4` and `-faststart-mp4` to keep MP4 inputs in MP4 while leaving other codecs in MKV. Before launching `ffmpeg`, opti inspects the probed stream to estimate bits-per-pixel and chooses engine-specific quality targets so HEVC outputs stay close to the original quality without runaway file sizes; add `-fast` if you want the heuristics to lean another step toward smaller encodes.
5. After successful encoding, the tool automatically replaces the original file with the transcoded version. The original is briefly renamed to `<name>.original` during the swap, then deleted once the new file is safely in place.
...
```bash
opti -s /srv/nas/series -w /srv/nas/opti-temp -I
```
